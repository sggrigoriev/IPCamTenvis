...
typedef struct OWData {
	...
	//CString sFileName;
	BOOL	bWriting;
	BOOL	bNeverWriteV, bNeverWriteA;
	DWORD	dwTsV0, dwTsA0, dwLastTsV, dwLastTsA;
	int		iOffsetA;
	DWORD	streamTypes;
...
static BOOL OurWriter_BeginWriting(const char *sFileName, DWORD streamTypes, void **ppData)
{
	...
	if( (pData->fp = fopen(sFileName, "wb")) )
	{
		...
		pData->dwTsV0 = pData->dwTsA0 = 0;
		pData->bNeverWriteV = pData->bNeverWriteA = TRUE;
		pData->bWriting = TRUE;
...
static BOOL OurWriter_Write(DWORD streamType, DWORD timeStamp, BYTE *buf, DWORD len, DWORD flag, void *data)
{
	...
	{
		if(MEDIATYPE_IS_VIDEO(streamType))
		{
			if(pWrtd->bNeverWriteV)
			{
				pWrtd->dwTsV0 = pWrtd->dwLastTsV = timeStamp;
				pWrtd->bNeverWriteV = FALSE;
				//pWrtd->vfhdr.tmStart = time(NULL);
			}
			dwTsDelta = timeStamp - pWrtd->dwLastTsV;
			//由于缓冲区满,帧被丢弃,delta值是可能较大的
			if(dwTsDelta > 80000) //前包后至。时标溢出前后的值与dwTs0相减其差是正常的,也可以此法判断
			{
				fprintf(stderr, "ts_delta = %d, timestamp confused....\n", dwTsDelta);
				return E_CONFUSED_TS;
			}
			pWrtd->dwLastTsV = timeStamp;
			timeStamp -= pWrtd->dwTsV0;
		}
		else if(MEDIATYPE_IS_AUDIO(streamType))
		{
			if(pWrtd->bNeverWriteA)
			{
				if(pWrtd->bNeverWriteV) return TRUE;
				//if(timeStamp < pWrtd->dwLastTsV) return TRUE;
				if((timeStamp < pWrtd->dwLastTsV) && (pWrtd->dwLastTsV - timeStamp) > 2000)
				{
					pWrtd->iOffsetA = (pWrtd->dwLastTsV - timeStamp);
				}
				else if((timeStamp > pWrtd->dwLastTsV) && (timeStamp - pWrtd->dwLastTsV) > 2000)
				{
					pWrtd->iOffsetA = -(timeStamp - pWrtd->dwLastTsV);
				}
				else
					pWrtd->iOffsetA = 0;
				pWrtd->dwTsA0 = pWrtd->dwLastTsA = timeStamp;
				pWrtd->bNeverWriteA = FALSE;
				//pWrtd->vfhdr.tmStart = time(NULL);
			}
			dwTsDelta = timeStamp - pWrtd->dwLastTsA;
			if(dwTsDelta > 20000) //前包后至。时标溢出前后的值与dwTs0相减其差是正常的,也可以此法判断
			{
				fprintf(stderr, "ts_delta = %d, audio timestamp confused....\n", dwTsDelta);
				return E_CONFUSED_TS;
			}

			pWrtd->dwLastTsA = timeStamp;
			timeStamp -= pWrtd->dwTsV0;
			timeStamp += pWrtd->iOffsetA;
			if(timeStamp > 0xFFFF0000) timeStamp = 0xFFFFFFFF - timeStamp;
		}


		do {
			int offset = ftell(pWrtd->fp);
			if(offset < 0) return E_WRITEFILE;

			
			if(fwrite(&streamType, 4, 1, pWrtd->fp) != 1) return E_WRITEFILE;
			len += 8;	//包含时标和标志
			if(fwrite(&len, sizeof(DWORD), 1, pWrtd->fp) != 1) return E_WRITEFILE;
			if(fwrite(&timeStamp, sizeof(DWORD), 1, pWrtd->fp) != 1) return E_WRITEFILE;
			if(fwrite(&flag, sizeof(DWORD), 1, pWrtd->fp) != 1) return E_WRITEFILE;
			fflush(pWrtd->fp);

			len -= 8;
			if(fwrite(buf, 1, len, pWrtd->fp) != len);
				return E_WRITEFILE;
			//fflush(pWrtd->fp);

			if( MEDIATYPE_IS_VIDEO(streamType) && (flag & STREAMFLAG_KEYFRAME) )
			{
...
/*TODO-GSG */
	else if(TagName == TAG_CHANNELID)	pWrtd->vfhdr.dwChannel = (intptr_t)Tag;
...
static void OurWriter_EndWriting(void *pData)
{
	if(pData)
	{
		...
		if(fp) 
		{
			...
			pWrtd->vfhdr.tmEnd = pWrtd->vfhdr.tmStart + (pWrtd->dwLastTsV-pWrtd->dwTsV0)/1000;
			rewind(fp);
